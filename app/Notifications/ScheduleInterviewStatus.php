...
class ScheduleInterviewStatus extends Notification
{
    ...
    public function toNexmo($notifiable)
    {
        return (new NexmoMessage)
                    ->content(
                        $this->jobApplication->full_name.' '.__('email.interviewScheduleStatus.text').' - ' . ucwords($this->jobApplication->job->title) .' ' .__('email.interviewScheduleStatus.statusChangesTo'). '  ' . $this->jobApplication->status->status
                    )->unicode();
    }
}
